...
print_metrics1(int id, Metrics *metrics, Configuration *conf)
{
  ...
  if (metrics->insert_ops) {
    ...
    printf("Insert: Total= %lu OPS= %f LHT= %f Latency (P0) %f (P50) %f (P90) %f (P95) %f (P99)%f (Last) %f \n", (long unsigned int)metrics->insert_ops, (double)metrics->insert_ops / metrics->insert_latency_total, metrics->total_lock_hold/2200000000.0 , metrics->latency_store[0], metrics->latency_store[p50], metrics->latency_store[p90], metrics->latency_store[p95], metrics->latency_store[p99], metrics->latency_store[metrics->insert_ops - 1]);
  }
  ...
  if (metrics->find_ops) {
    ...
    printf("Find: Total= %lu OPS= %f LHT= %f Latency (P0) %f (P50) %f (P90) %f (P95) %f (P99) %f (Last) %f \n", (long unsigned int)metrics->find_ops, (double)metrics->find_ops / metrics->find_latency_total, metrics->find_total_lock_hold/2200000000.0 , metrics->find_latency_store[0], metrics->find_latency_store[p50], metrics->find_latency_store[p90], metrics->find_latency_store[p95], metrics->find_latency_store[p99], metrics->find_latency_store[metrics->find_ops - 1]);
  }
...
run_single_test(Configuration *conf)
{
  ...
  all_latency = new double[total_ops];
  find_all_latency = new double[find_total_ops];
