    ...
    m_cursor(0), m_progress(0), m_success(true), m_erase_only(false)
{
  ...
  m_metrics.latency_store = new double[1000000000];
  m_metrics.find_latency_store = new double[1000000000];
...
bool
RuntimeGenerator::execute(int i)
{
...
RuntimeGenerator::insert()
{
  ...
  unsigned long long lht = 0;
  //start = rdtscp();
  if (m_cursor)
    m_last_status = m_db->cursor_insert(m_cursor, &key, &rec);
  else
    m_last_status = m_db->insert(m_txn, &key, &rec, &lht, NULL);
   //printf("insert elapsed: %lld\n", elapsed);
  //end = rdtscp();
  double elapsed = t.seconds();

  //std::cout << std::fixed << boost::this_thread::get_id() << " - Latency " << elapsed << " \n";
  ...
  m_metrics.latency_store[m_metrics.insert_ops] = elapsed;
  m_metrics.total_lock_hold += lht;
  //m_metrics.latency_store[m_metrics.insert_ops] = (end - start) / 2400;
...
RuntimeGenerator::find()
{
  ...
  if (m_cursor)
    ...
    m_last_status = m_db->find(m_txn, &key, &m_record, &lht, NULL);
  ...
  m_metrics.find_ops++;
  m_metrics.find_latency_store[m_metrics.find_ops] = elapsed;
  m_metrics.find_total_lock_hold += lht;
...
int
RuntimeGenerator::get_next_command(int i)
{
