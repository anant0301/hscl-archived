...
static void
print_metrics1(int id, Metrics *metrics, Configuration *conf)
{
  ...
  std::sort(metrics->latency_store, metrics->latency_store + metrics->insert_ops);  
  std::sort(metrics->find_latency_store, metrics->find_latency_store + metrics->find_ops);  

  /*char my_file[2];
  my_file[0] = id + '0';
  my_file[1] = '\0';
  FILE *latency_file = fopen(my_file, "w");
  if (id % 2) {
  for(int i = 0; i < metrics->insert_ops; i++) {
	fprintf(latency_file, "%lld,", (unsigned long long)(metrics->latency_store[i] * 2400L * 1000000));
  }
  } else {
  for(int i = 0; i < metrics->find_ops; i++) {
    fprintf(latency_file, "%lld,", (unsigned long long)(metrics->find_latency_store[i] * 2400L * 1000000));
  }
  }*/
  ...
  if (metrics->insert_ops) {
    int p50 = metrics->insert_ops * 0.5;
    int p90 = metrics->insert_ops * 0.9;
    int p95 = metrics->insert_ops * 0.95;
    int p99 = metrics->insert_ops * 0.99;
    printf("Thread %d Insert: Total= %lu OPS= %f LHT= %f Latency (P0) %f (P50) %f (P90) %f (P95) %f (P99)%f (Last) %f \n", t_i, (long unsigned int)metrics->insert_ops, (double)metrics->insert_ops / metrics->insert_latency_total, metrics->total_lock_hold/2200000000.0 , metrics->latency_store[0], metrics->latency_store[p50], metrics->latency_store[p90], metrics->latency_store[p95], metrics->latency_store[p99], metrics->latency_store[metrics->insert_ops - 1]);
  }

  if (metrics->find_ops) {
    int p50 = metrics->find_ops * 0.5;
    int p90 = metrics->find_ops * 0.9;
    int p95 = metrics->find_ops * 0.95;
    int p99 = metrics->find_ops * 0.99;
    printf("Thread %d Find: Total= %lu OPS= %f LHT= %f Latency (P0) %f (P50) %f (P90) %f (P95) %f (P99) %f (Last) %f \n", t_i, (long unsigned int)metrics->find_ops, (double)metrics->find_ops / metrics->find_latency_total, metrics->find_total_lock_hold/2200000000.0 , metrics->find_latency_store[0], metrics->find_latency_store[p50], metrics->find_latency_store[p90], metrics->find_latency_store[p95], metrics->find_latency_store[p99], metrics->find_latency_store[metrics->find_ops - 1]);
  }
...
struct Callable
{
  ...
  void operator()(int i) {
   // std::cout << boost::this_thread::get_id() << " i = " << i << "\n";
	cpu_set_t cpuset;
    CPU_ZERO(&cpuset);
    /*if (i % 2 == 0) {
        CPU_SET(i / 2, &cpuset);
    } else {
    CPU_SET(i / 2 + 10, &cpuset);
    }*/
    CPU_SET(i, &cpuset);
    pthread_setaffinity_np(pthread_self(), sizeof(cpu_set_t), &cpuset);
    while (m_generator->execute(i));
    ...
    std::cout << "Sched: " << i << " " << tid << " " << buffer << "\n";
    //std::cout << boost::this_thread::get_id() << " " << tid << " " << buffer << "\n";
  }
...
static void
thread_callback(Callable *c, int i)
{
  return (*c)(i);
}
...
run_single_test(Configuration *conf)
{
  ...
  /*uint64_t stop_seconds = conf->limit_seconds;
  conf->limit_ops = 1000000;
  conf->limit_seconds = 0;
  GeneratorType temp_generator(0, conf, db, true);
  while(temp_generator.execute(1));
  // create additional upscaledb threads
  conf->limit_ops = 0;
  conf->limit_seconds = stop_seconds;*/
  GeneratorType generator(0, conf, db, true);
  ...
  for (int i = 1; i < conf->num_threads; i++) {
    ...
    threads.push_back(new boost::thread(thread_callback, c, i));
  }
  ...
  while (generator.execute(0)) {
    ...
    std::cout  << "Sched: 0 " << tid << " " << buffer << "\n";

  double *all_latency = NULL;
  double *find_all_latency = NULL;
  uint64_t total_ops = 0;
  uint64_t find_total_ops = 0;

  int ops_counter = 0;
  int find_ops_counter = 0;
  // have to collect metrics now while the database was not yet closed
  Metrics metrics;
  generator.get_metrics(&metrics);
  std::cout << "Thread 0 stats (" << boost::this_thread::get_id() << ") \n";
  //printf("Thread 0 stats\n");
  print_metrics1(0,&metrics, conf);

  total_ops += metrics.insert_ops;
  find_total_ops += metrics.find_ops;

  std::vector<boost::thread *>::iterator it1;
  std::vector<Callable *>::iterator cit1 = callables.begin();
  for (it1 = threads.begin(); it1 != threads.end(); it1++, cit1++) {
    Metrics m;
    (*cit1)->get_metrics(&m);
    total_ops += m.insert_ops;
    if (m.find_ops) {
    	find_total_ops += m.find_ops;
    }
  } 

  all_latency = new double[2*total_ops];
  find_all_latency = new double[2*find_total_ops];

  for (int i = 0; i < metrics.insert_ops; i++)
  {
	all_latency[ops_counter] = metrics.latency_store[i];
	ops_counter++;
  }

  if (metrics.find_ops) {
	  for (int a = 0; a < metrics.find_ops; a++)
	  {
		find_all_latency[find_ops_counter] = metrics.find_latency_store[a];
		find_ops_counter++;
	  }
  }
  ...
  for (it = threads.begin(); it != threads.end(); it++, cit++) {
    Metrics m;
    (*cit)->get_metrics(&m);
    std::cout << "Thread " << t_i << " stats (" << boost::this_thread::get_id() << ") \n";
    //printf("Thread %d stats\n", i);
    print_metrics1(t_i, &m, conf);
    for (int k = 0; k < m.insert_ops; k++)
    {
	all_latency[ops_counter] = m.latency_store[k];
	ops_counter++;
    }
    if (m.find_ops) {
	    for (int l = 0; l < m.find_ops; l++)
	    {
		find_all_latency[find_ops_counter] = m.find_latency_store[l];
		find_ops_counter++;
	    }
    }
  ...
  if (ok) {
    ...
    if (!conf->quiet || conf->metrics != Configuration::kMetricsNone) {
      std::sort(all_latency, all_latency + total_ops);
      int p50 = total_ops * 0.5;
      int p90 = total_ops * 0.9;
      int p95 = total_ops * 0.95;
      int p99 = total_ops * 0.99;
      printf("%f %f %f %f %f %f \n", all_latency[0], all_latency[p50], all_latency[p90], all_latency[p95], all_latency[p99], all_latency[total_ops - 1]);

      if(find_total_ops) {
	      std::sort(find_all_latency, find_all_latency + find_total_ops);
	      p50 = find_total_ops * 0.5;
	      p90 = find_total_ops * 0.9;
	      p95 = find_total_ops * 0.95;
	      p99 = find_total_ops * 0.99;
	      printf("%f %f %f %f %f %f \n", find_all_latency[0], find_all_latency[p50], find_all_latency[p90], find_all_latency[p95], find_all_latency[p99], find_all_latency[find_total_ops - 1]);
      }
...
run_both_tests(Configuration *conf)
{
  ...
  uint64_t op = 0;
  while (generator1.execute(0)) {
    bool b = generator2.execute(0);
    assert(b);
    op++;
  ...
  if (ok)
    assert(false == generator2.execute(0));
