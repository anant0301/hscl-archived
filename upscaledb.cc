...
UpscaleDatabase::do_insert(Transaction *txn, ups_key_t *key,
                ups_record_t *record, unsigned long long *lht, unsigned long long *elapsed)
{
  ...
  ups_status_t st = ups_db_insert1(m_db, (ups_txn_t *)txn, key, record, flags, lht, elapsed);
  if (st)
...
ups_status_t
UpscaleDatabase::do_find(Transaction *txn, ups_key_t *key, ups_record_t *record, unsigned long long *lht, unsigned long long *elapsed)
{
  uint32_t flags = 0;
  ...
  ups_status_t st = ups_db_find1(m_db, (ups_txn_t *)txn, key, record, flags, lht, elapsed);
  if (st)
